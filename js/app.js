...
function saveProgress() {
    try {
        localStorage.setItem('janeEyreProgress', JSON.stringify(state.completedLessons));
    } catch (e) {
...
function getMultipleChoiceOptions(question, lessonId) {
    ...
    const questionMap = {
        ...
        'Where did Jane live after her parents died?': {
            options: ['With her grandparents', 'With her aunt and uncle', 'At a school', 'Alone'],
            correct: 1
        },
        'Was Jane happy at Gateshead Hall?': {
            options: ['Yes, she was very happy', 'No, she was unhappy', 'Sometimes happy', 'She didn\'t care'],
            correct: 1
        },
        // Lesson 2 questions
        'What did John do to Jane?': {
            options: ['He hugged her', 'He hit her with a book', 'He gave her a gift', 'He ignored her'],
            correct: 1
        },
        'How did Jane react when John hit her?': {
            options: ['She cried', 'She fought back', 'She ran away', 'She told her aunt'],
            correct: 1
        },
        'What did Jane say she wanted to do?': {
            options: ['Stay at Gateshead', 'Leave Gateshead', 'Go to school', 'Run away'],
            correct: 1
        },
        // Lesson 3 questions
        'Who was Mr. Brocklehurst?': {
            options: ['A teacher', 'The owner of the school', 'Jane\'s uncle', 'A doctor'],
            correct: 1
        },
        'What did Mr. Brocklehurst say about bad children?': {
            options: ['He liked them', 'God punishes them', 'They are funny', 'He ignores them'],
            correct: 1
        },
        ...
        'What was Lowood School like?': {
            options: ['Warm and comfortable', 'Cold and strict', 'Fun and exciting', 'Very luxurious'],
            correct: 1
        },
        'What did the girls wear at Lowood?': {
            options: ['Pretty dresses', 'Plain brown dresses', 'Fancy uniforms', 'Colorful clothes'],
            correct: 1
        },
        ...
        'Why did Mr. Brocklehurst punish Jane?': {
            options: ['She was late', 'She dropped her book', 'She was a liar', 'She was lazy'],
            correct: 2
        },
        ...
        'What did Mr. Brocklehurst have to do after the girls got sick?': {
            options: ['Buy better food and clothes', 'Close the school', 'Run away', 'Become a teacher'],
            correct: 0
        },
        ...
        'Who was Adele Varens?': {
            options: ['Jane\'s pupil', 'Mrs. Fairfax\'s daughter', 'Mr. Rochester\'s wife', 'A servant'],
            correct: 0
        },
        'Who was Mr. Rochester?': {
            options: ['The owner of Thornfield', 'A teacher', 'A doctor', 'Jane\'s cousin'],
            correct: 0
        },
        'What was Thornfield Hall like?': {
            options: ['Small and old', 'Beautiful with large windows', 'Dark and scary', 'Modern and new'],
            correct: 1
        },
        // Lesson 9 questions
        'What happened to the man on the horse?': {
            options: ['He fell and hurt his foot', 'He lost his horse', 'He was angry', 'He ran away'],
            correct: 0
        },
        ...
        'What did Mr. Rochester look like?': {
            options: ['Handsome and young', 'Strong face, dark eyes', 'Tall and thin', 'Short and fat'],
            correct: 1
        },
        'How did Jane help Mr. Rochester?': {
            options: ['She gave him water', 'She helped him onto his horse', 'She called a doctor', 'She went to get help'],
            correct: 1
        },
        ...
        'What did Jane hear in the night?': {
            options: ['A strange laugh', 'A door slamming', 'Someone crying', 'A fire alarm'],
            correct: 0
        },
        'Where was the smoke coming from?': {
            options: ['The kitchen', 'Mr. Rochester\'s room', 'The garden', 'The library'],
            correct: 1
        },
        ...
        'What did Mr. Rochester ask Jane in the garden?': {
            options: ['To leave Thornfield', 'To marry him', 'To find a new job', 'To help Adele'],
            correct: 1
        },
        'Why did Jane think she had to leave Thornfield?': {
            options: ['She was fired', 'Mr. Rochester was marrying Miss Ingram', 'She was homesick', 'She found a better job'],
            correct: 1
        },
        ...
        'What happened on Jane\'s wedding day?': {
            options: ['She got married', 'The wedding was stopped', 'She ran away', 'Mr. Rochester didn\'t show up'],
            correct: 1
        },
        'Who stopped the wedding?': {
            options: ['A lawyer named Briggs', 'The clergyman', 'Mrs. Fairfax', 'Grace Poole'],
            correct: 0
        },
        'What did the man in the church say?': {
            options: ['The couple must not marry', 'The wedding can go on', 'Mr. Rochester is not rich', 'Jane is too young'],
            correct: 0
        },
        // Lesson 14 questions
        'What secret did Mr. Rochester have?': {
            options: ['He was poor', 'He had a wife', 'He was a criminal', 'He was not the owner'],
            correct: 1
        },
        'Who was Richard Mason?': {
            options: ['Mr. Rochester\'s brother', 'Bertha Mason\'s brother', 'The clergyman', 'A lawyer'],
            correct: 1
        },
        'Where was Mrs. Rochester living?': {
            options: ['In London', 'At Thornfield Hall', 'In the West Indies', 'At Gateshead Hall'],
            correct: 1
        },
        // Lesson 15 questions
        'What happened to Thornfield Hall?': {
            options: ['It was sold', 'It burned down', 'It was rebuilt', 'It was abandoned'],
            correct: 1
        },
        'What happened to Mr. Rochester?': {
            options: ['He died in the fire', 'He was blinded and lost a hand', 'He ran away', 'He went to London'],
            correct: 1
        },
...
function renderLessonList() {
    ...
    let html = '<h3 style="margin-top: 0; margin-bottom: 10px; font-size: 18px;">📚 Lessons</h3>';
    
    state.lessons.forEach(lesson => {
        const isCompleted = state.completedLessons.includes(lesson.id);
        const isActive = state.currentLessonId === lesson.id;
        const isLocked = !isCompleted && lesson.id > 1 && !state.completedLessons.includes(lesson.id - 1);
...
function loadLesson(lessonId) {
    ...
    const isPreviousCompleted = lessonId === 1 || state.completedLessons.includes(lessonId - 1);
...
function renderLesson(lesson) {
    ...
    for (let i = 0; i < totalQuestions; i++) {
        ...
        if (answeredQuestions[key] !== undefined) {
            answeredCount++;
            const qData = getMultipleChoiceOptions(lesson.comprehension[i], lesson.id);
...
function updateProgress() {
    ...
    if (DOM.progressText) DOM.progressText.textContent = `${completed} / ${total} lessons`;
    ...
    if (e.key === 'ArrowRight' || e.key === 'ArrowDown') {
        ...
        if (current && current < state.lessons.length) {
            if (state.completedLessons.includes(current)) {
                ...
                showToast('📚 Complete this lesson first before moving to the next one.');
            }
        }
...
function init() {
    ...
    console.log('⌨️ Keyboard shortcuts: ← / ↑ = Previous lesson, → / ↓ = Next lesson (if completed)');
...
function showWelcomeMessage() {
    ...
    if (!hasVisited) {
        ...
        console.log('📚 Complete lessons in order. Each lesson unlocks the next one.');
